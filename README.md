...
主頁、列表、卡片、商品清單等會一次顯示多張圖片的畫面，統一採用：

> **畫面先載入輕量縮圖；使用者點擊放大時，才載入原圖／高解析圖。**

正式規則：

1. **主頁／列表／卡片的 `<img>` 預設只能使用縮圖，不直接使用原圖。**
2. 縮圖應搭配 `loading="lazy"`、`decoding="async"` 等既有延遲載入機制，避免初始頁面同時下載大量圖片。
3. 原圖／高解析圖必須保留，提供 modal、lightbox、點圖放大等互動使用。
4. **只有使用者實際點擊圖片時，才切換或載入原圖；不可在頁面初始化時預載全部原圖。**
5. 縮圖只負責主頁／列表顯示；**放大畫面禁止拿縮圖直接拉伸**，避免馬賽克與模糊，必須使用原圖／高解析版本。
6. 已有 `thumb` / `large`、`image` / `imageFull`、`data-product-thumbnail` / `data-product-image` 等雙圖來源架構時，後續修改應沿用，不要退回單一原圖直接載入。
7. 更換商品圖、首頁圖或其他可放大圖片時，若該功能已有縮圖／原圖雙軌機制，**必須同步確認縮圖與原圖映射都正確**，不可只換其中一邊造成縮圖與放大圖不一致。
8. 原始高解析圖片不可因為製作縮圖而刪除或覆蓋；縮圖屬於顯示效能資產，原圖仍是放大檢視的正式來源。
9. 若產生 WebP 等最佳化縮圖，屬於效能衍生檔，可以保留原始 PNG／JPG 作為高解析來源；不要為了縮圖最佳化犧牲放大圖品質。
